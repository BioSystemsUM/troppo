...
if __name__ == '__main__':
	...
	MODEL_PATH = '../troppo/tests/Recon3D_301.xml' # SBML model path
	DATA_PATH = '../troppo/tests/ccle_breast_cancer/CCLE_breast_cancer_preprocessed.csv' # tabular csv with expression
	TASKS_PATH = '../troppo/tests/nl2019_tasks_r3d_compact.json' # JSON containing metabolic tasks
	CMODL_PATH = '../troppo/tests/Recon3D_301_consistent.xml' # Consistent SBML model path - recommended!
	TASK_RESULTS_PATH = '../troppo/tests/r3d_compact_task_results_ccle_bc_new_nodrains_only_feas.json' # task evaluation
	CS_MODEL_DF_PATH = '../troppo/tests/r3d_compact_ccle_bc_fastcore.csv' # context-specific models extracted from algos
	...
	for k, result in fastcore_res_dict.items():
		...
		print(k,':',sum([v[0] for k,v in task_csm_res.items()]),'tasks completed')
		## assign this dictionary to it's sample on the master results dictionary
		task_eval_results[k] = task_csm_res

	#task_eval_results_slim = [(k,{tn:tv for tn,tv in v.items()}) for k,v in task_eval_results.items()]

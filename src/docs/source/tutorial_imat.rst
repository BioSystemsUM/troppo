...
For the `ContinuousScoreIntegrationStrategy` the output will be a dictionary with reaction names as keys and scores as values, which is a requirement for the *iMAT* method.
...
Hence, if you use this strategy, *iMAT* will not be able to run.
...
In the end, the `run()` method of the `IMAT` class will return a list of the active reactions index that should be keep in the final model.
